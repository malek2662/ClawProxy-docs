# OpenCode & Kilo CLI Integration Guide

This document explains how to route AI requests through **ClawProxy** using **OpenCode CLI** or **Kilo CLI**. Both tools share the same configuration format, so everything here applies to both.
...
|-------|----------|-------------|
| `npm` | Yes | AI SDK package to use. See [SDK Packages](#sdk-packages) below. |
...
These providers offer genuine free tiers. You need a free API key from each provider.
...
2. **API Keys** tab → add `sk-not-required` as the key.
3. Copy the **Base URL**.
...
You can add multiple providers in a single config file. This gives you access to all of them from the CLI's provider selector.
...
> **Tip:** You can mix free-tier providers with paid providers — all in the same config file. Each provider appears as a separate option in the CLI's provider selector.
...
4. **Look up model IDs and limits** from the provider's documentation, or use **Fetch Models** in the ClawProxy dashboard to see available models.
