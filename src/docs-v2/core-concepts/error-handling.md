...
| **QUOTA_EXHAUSTED** | 429, 403, 400, 401 | Window-quota bodies: "usage limit", "access_terminated", "billing cycle", "quota will be refreshed", "quota exceeded", "usage_limit_reached"; Z.AI codes 1308 / 1310 / 1316-1321 | Window-accurate backoff on key (until its own window's reset), **never disabled**, try next key |
...
| **Quota Exhausted** (window quota: Kimi Coding 5h/weekly/monthly cycles) | Long cooldown -- **never disabled** | Until the key's own exhausted window resets (see below) | Key Rate Limited |
...
| **Kimi Coding** | Returns HTTP 403 `access_terminated_error` (not 429) when a 5-hour, weekly, or monthly quota window is full | Classified as QUOTA_EXHAUSTED. Key backed off until its own exhausted window's reset (per-key snapshot), never disabled. |
